...
/**
 * @param configId is the unique ID for this config. Typically UUID.random().toString()
 ...
 * @param scaleTime - duration of scaling animation. Use [startScale] and [targetScale] to control the parameters of particle scale
 * @param colors - a list of colors that each particle will randomly pick from
 * @param particleSizes - a list of sizes in [DpSize] for each particle. It will be picked randomly from available sizes. The size can be also manipulated using [startScale] and [targetScale]
 ...
 * @param targetScale - end scale for each particle
 * @param startScale - start scale for each particle
 * @param hideInStartRegion - this is useful for our Boost. We are hiding the particles in the start region and use blurred images there instead
 */
